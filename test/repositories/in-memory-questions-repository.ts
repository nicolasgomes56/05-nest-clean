...
export class InMemoryQuestionsRepository implements QuestionsRepository {
  ...
  async findDetailsBySlug(slug: string) {
    ...
    if (!author) throw new Error(`Author with ID ${question.authorId.toString()} does not found.`);

    const questionsAttachments = this.questionAttachmentsRepository.items.filter(
      (questionAttachments) => {
    ...
    const attachments = questionsAttachments.map((questionAttachment) => {
      ...
      if (!attachment) {
        throw new Error(
          `Attachment with ID ${questionAttachment.attachmentId.toString()} does not found.`,
        );
      }
